...
class DynamodbGateway:
    ...
    def scan_table(cls, table_name, last_evaluated_key=None):
        ...
        if 'Items' not in response:
            raise Exception("There is no objects for this object on table {}".format(table_name))
        ...
        while ("LastEvaluatedKey" in response) and (response["LastEvaluatedKey"] != None):
            ...
            print(response)
            print("item_count: {}, LastEvaluatedKey: {}".format(len(items), response.get('LastEvaluatedKey')))
    ...
    def query_by_partition_key(cls, table_name, partition_key_name, partition_key_query_value, attributes="ALL_ATTRIBUTES"):
        print("Reading from table {}".format(table_name))
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table(table_name)
        items = []
        ...
        print(resp)
        print("item_count: {}, LastEvaluatedKey: {}".format(len(items), resp.get('LastEvaluatedKey')))
